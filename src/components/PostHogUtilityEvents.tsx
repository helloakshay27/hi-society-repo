...
export const useUtilityEvents = () => {
    ...
    const safeCapture = (eventName: string, properties: any = {}) => {
        ...
        // Data-quality guard
        if (properties.utility_name && properties.utility_name.match(/test|tesr|oiuytdftyj/i)) {
    ...
    const onUtilityCreated = (properties: {
        utility_id: string;
        type?: string;
        status?: string;
        [key: string]: any;
    }) => {
        safeCapture('Utility Created', properties);
    };

    const onUtilityUpdated = (properties: {
        utility_id: string;
        status?: string;
        [key: string]: any;
    }) => {
        safeCapture('Utility Updated', properties);
    };

    const onUtilityDeleted = (properties: {
        utility_id: string;
        reason?: string;
        [key: string]: any;
    }) => {
        safeCapture('Utility Deleted', properties);
    };

    const onUtilityViewed = (properties: {
        utility_id: string;
        [key: string]: any;
    }) => {
        safeCapture('Utility Viewed', properties);
    };
