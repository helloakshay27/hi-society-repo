...
export const useAccountingEvents = () => {
    ...
    const safeCapture = (eventName: string, properties: any = {}) => {
        ...
        // Data-quality guard
        if (properties.accounting_name && properties.accounting_name.match(/test|tesr|oiuytdftyj/i)) {
    ...
    const onAccountingCreated = (properties: {
        accounting_id: string;
        type?: string;
        status?: string;
        [key: string]: any;
    }) => {
        safeCapture('Accounting Created', properties);
    };

    const onAccountingUpdated = (properties: {
        accounting_id: string;
        status?: string;
        [key: string]: any;
    }) => {
        safeCapture('Accounting Updated', properties);
    };

    const onAccountingDeleted = (properties: {
        accounting_id: string;
        reason?: string;
        [key: string]: any;
    }) => {
        safeCapture('Accounting Deleted', properties);
    };

    const onAccountingViewed = (properties: {
        accounting_id: string;
        [key: string]: any;
    }) => {
        safeCapture('Accounting Viewed', properties);
    };
