  ...
  Safety: [
    {
      ...
      subItems: [
        ...
        {
          name: "Krcc List",
          href: "/safety/m-safe/krcc-list",
          color: "text-[#1a1a1a]",
        },
        {
          name: "Training List",
          href: "/safety/m-safe/training-list",
          color: "text-[#1a1a1a]",
        },
        {
          name: "External Reportee Reassign",
...
const ViSidebar: React.FC = () => {
        ...
        <div key={item.name} className="w-full">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              {level === 0 && item.icon && (
                <item.icon
                  className={`w-5 h-5 ${isActive || hasActiveChild ? "text-[#C72030]" : ""}`}
                />
              )}

              <span
                className={`${level === 0 && (isActive || hasActiveChild) ? "text-[#C72030]" : ""}`}
              >
                {item.name}
              </span>
            </div>
        ...
        >
          {level === 0 && (
            <>
              ...
              {item.icon && (
                <item.icon
                  className={`w-5 h-5 ${isActive ? "text-[#C72030]" : ""}`}
                />
              )}
            </>
          )}
  ...
  return (
    <div
      className={`${isSidebarCollapsed ? "w-16" : "w-64"} bg-[#f6f4ee] border-r border-[#D5DbDB] fixed left-0 top-0 overflow-y-auto transition-all duration-300 z-40 ${isMobileSidebarOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0`}
