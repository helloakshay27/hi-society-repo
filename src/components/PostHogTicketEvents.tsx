...
export const useTicketEvents = () => {
    ...
    const safeCapture = (eventName: string, properties: any = {}) => {
        ...
        // Data-quality guard
        if (properties.ticket_name && properties.ticket_name.match(/test|tesr|oiuytdftyj/i)) {
    ...
    const onTicketCreated = (properties: {
        ticket_id: string;
        type?: string;
        status?: string;
        [key: string]: any;
    }) => {
        safeCapture('Ticket Created', properties);
    };

    const onTicketUpdated = (properties: {
        ticket_id: string;
        status?: string;
        [key: string]: any;
    }) => {
        safeCapture('Ticket Updated', properties);
    };

    const onTicketDeleted = (properties: {
        ticket_id: string;
        reason?: string;
        [key: string]: any;
    }) => {
        safeCapture('Ticket Deleted', properties);
    };

    const onTicketViewed = (properties: {
        ticket_id: string;
        [key: string]: any;
    }) => {
        safeCapture('Ticket Viewed', properties);
    };
