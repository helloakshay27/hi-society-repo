...
export const useAssetEvents = () => {
    ...
    const safeCapture = (eventName: string, properties: any = {}) => {
        ...
        // Data-quality guard
        if (properties.asset_name && properties.asset_name.match(/test|tesr|oiuytdftyj/i)) {
    ...
    const onAssetCreated = (properties: {
        asset_id: string;
        type?: string;
        status?: string;
        [key: string]: any;
    }) => {
        safeCapture('Asset Created', properties);
    };

    const onAssetUpdated = (properties: {
        asset_id: string;
        status?: string;
        [key: string]: any;
    }) => {
        safeCapture('Asset Updated', properties);
    };

    const onAssetDeleted = (properties: {
        asset_id: string;
        reason?: string;
        [key: string]: any;
    }) => {
        safeCapture('Asset Deleted', properties);
    };

    const onAssetViewed = (properties: {
        asset_id: string;
        [key: string]: any;
    }) => {
        safeCapture('Asset Viewed', properties);
    };
