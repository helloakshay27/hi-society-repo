...
export const useCampaignsEvents = () => {
    ...
    const safeCapture = (eventName: string, properties: any = {}) => {
        ...
        // Data-quality guard
        if (properties.campaigns_name && properties.campaigns_name.match(/test|tesr|oiuytdftyj/i)) {
    ...
    const onCampaignsCreated = (properties: {
        campaigns_id: string;
        type?: string;
        status?: string;
        [key: string]: any;
    }) => {
        safeCapture('Campaigns Created', properties);
    };

    const onCampaignsUpdated = (properties: {
        campaigns_id: string;
        status?: string;
        [key: string]: any;
    }) => {
        safeCapture('Campaigns Updated', properties);
    };

    const onCampaignsDeleted = (properties: {
        campaigns_id: string;
        reason?: string;
        [key: string]: any;
    }) => {
        safeCapture('Campaigns Deleted', properties);
    };

    const onCampaignsViewed = (properties: {
        campaigns_id: string;
        [key: string]: any;
    }) => {
        safeCapture('Campaigns Viewed', properties);
    };
