...
export const AssetTableDisplay: React.FC<AssetTableDisplayProps> = ({
  selectedAssets,
}) => {
              ...
              {selectedAssets.map((asset, index) => (
                <tr key={asset.id || index} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-sm text-gray-600">
                    {asset.asset_number || asset.id || "N/A"}
                  </td>
                  ...
                  <td className="px-4 py-3 text-sm text-gray-600">
                    {asset.building?.name || "N/A"}
                  </td>
                  <td className="px-4 py-3 text-sm text-gray-600">
                    {asset.wing?.name || "N/A"}
                  </td>
                  <td className="px-4 py-3 text-sm text-gray-600">
                    {asset.floor?.name || "N/A"}
                  </td>
                  <td className="px-4 py-3 text-sm text-gray-600">
                    {asset.area?.name || "N/A"}
                  </td>
                  <td className="px-4 py-3 text-sm text-gray-600">
                    {asset.pmsRoom?.name || "N/A"}
                  </td>
                </tr>
              ))}
            </tbody>
