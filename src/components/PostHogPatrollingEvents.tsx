...
export const usePatrollingEvents = () => {
    ...
    const safeCapture = (eventName: string, properties: any = {}) => {
        ...
        // Data-quality guard
        if (properties.patrolling_name && properties.patrolling_name.match(/test|tesr|oiuytdftyj/i)) {
    ...
    const onPatrollingCreated = (properties: {
        patrolling_id: string;
        type?: string;
        status?: string;
        [key: string]: any;
    }) => {
        safeCapture('Patrolling Created', properties);
    };

    const onPatrollingUpdated = (properties: {
        patrolling_id: string;
        status?: string;
        [key: string]: any;
    }) => {
        safeCapture('Patrolling Updated', properties);
    };

    const onPatrollingDeleted = (properties: {
        patrolling_id: string;
        reason?: string;
        [key: string]: any;
    }) => {
        safeCapture('Patrolling Deleted', properties);
    };

    const onPatrollingViewed = (properties: {
        patrolling_id: string;
        [key: string]: any;
    }) => {
        safeCapture('Patrolling Viewed', properties);
    };
