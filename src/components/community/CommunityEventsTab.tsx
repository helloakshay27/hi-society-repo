...
const CommunityEventsTab = ({ communityId }: CommunityEventsTabProps) => {
    ...
    const fetchNotices = async () => {
      try {
        const response = await axios.get(
          `https://${baseUrl}/communities/${communityId}/notices.json`,
          {
            headers: {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        const response = await axios.get(
          `https://${baseUrl}/communities/${communityId}/events.json`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  useEffect(() => {
    const fetchDocuments = async () => {
      try {
        const response = await axios.get(
          `https://${baseUrl}/folders/by_communities.json?community_ids=[${communityId}]`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
        ...
        setDocuments(
          response.data.folders.flatMap((folder: any) => folder.documents) || []
        );
      } catch (error) {
        console.log(error);
      }
  ...
  const handleVisibleAfterExpireChange = async (
    item: any,
    checked: boolean
  ) => {
    const newStatus = checked ? 1 : 0;
  ...
  const renderDocumentActions = (document: Document) => (
    <Button
      variant="ghost"
      size="sm"
      onClick={() => navigate(`/pulse/community/document/${document.id}`)}
    >
  ...
  const renderEventActions = (event: Event) => (
    <Button
      variant="ghost"
      size="sm"
      onClick={() => navigate(`/pulse/community/event/${event.id}`)}
    >
      <Eye className="w-4 h-4" />
    </Button>
  );
  ...
  const handleEventStatusChange = async (item: any, checked: boolean) => {
    ...
    try {
      ...
      setEvents((prev) =>
        prev.map((event) =>
          event.id === item.id
            ? { ...event, active: item.active !== false }
            : event
        )
      );
    } finally {
  ...
  const renderDocumentCell = (document: Document, columnKey: string) => {
    ...
    if (columnKey === "format") {
      return (
        document.attachment.file_type.charAt(0).toUpperCase() +
        document.attachment.file_type.slice(1).toLowerCase()
      );
    }
    if (columnKey === "size") {
      ...
      return (
        <div className="flex items-center gap-2">
          <Switch
            checked={isChecked}
            onChange={(e) =>
              handleDocumentStatusChange(document, e.target.checked)
            }
      ...
      return (
        <div className="flex items-center gap-2">
          <Switch
            ...
            onChange={(e) =>
              handleShowOnHomeScreenChange(notice, e.target.checked)
            }
      ...
      return (
        <div className="flex items-center gap-2">
          <Switch
            ...
            onChange={(e) =>
              handleVisibleAfterExpireChange(notice, e.target.checked)
            }
              ...
              <div className="flex flex-col justify-center px-3 py-2 flex-1">
                ...
                <span className="text-[12px] font-medium text-[#6B7280] break-words leading-tight">
                  {selectedDocuments.length === 1
                    ? documents.find(
                        (doc) => String(doc.id) === selectedDocuments[0]
                      )?.title || "Document"
        ...
        <DialogContent className="max-w-sm bg-white rounded-lg p-0 flex flex-col border-0 shadow-lg">
          <div className="bg-white pt-12 text-center flex flex-col">
            <h2 className="text-base font-semibold text-gray-900 mb-12 leading-tight">
              ...
              <br />
              the selected document{selectedDocuments.length > 1 ? "s" : ""}?
            </h2>
              ...
              <div className="flex flex-col justify-center px-3 py-2 flex-1">
                ...
                <span className="text-[12px] font-medium text-[#6B7280] break-words leading-tight">
                  {selectedNotices.length === 1
                    ? notices.find(
                        (notice) => String(notice.id) === selectedNotices[0]
                      )?.title || "Notice"
                    : selectedNotices.length <= 3
                      ? selectedNotices
                          .map(
                            (id) =>
                              notices.find((notice) => String(notice.id) === id)
                                ?.title || "Notice"
                          )
                          .join(", ")
                      : `${selectedNotices
                          ...
                          .join(", ")} and ${selectedNotices.length - 3} more`}
                </span>
              </div>
            </div>
        ...
        <DialogContent className="max-w-sm bg-white rounded-lg p-0 flex flex-col border-0 shadow-lg">
          <div className="bg-white pt-12 text-center flex flex-col">
            <h2 className="text-base font-semibold text-gray-900 mb-12 leading-tight">
              Are you sure you want to delete
              <br />
              the selected notice{selectedNotices.length > 1 ? "s" : ""}?
            </h2>
            <div className="flex mt-auto">
              <button
              ...
              <div className="flex flex-col justify-center px-3 py-2 flex-1">
                ...
                <span className="text-[12px] font-medium text-[#6B7280] break-words leading-tight">
                  {selectedEvents.length === 1
                    ? events.find(
                        (event) => String(event.id) === selectedEvents[0]
                      )?.title || "Event"
                    : selectedEvents.length <= 3
                      ...
                      : `${selectedEvents
                          ...
                          .join(", ")} and ${selectedEvents.length - 3} more`}
                </span>
              </div>
            </div>
        ...
        <DialogContent className="max-w-sm bg-white rounded-lg p-0 flex flex-col border-0 shadow-lg">
          <div className="bg-white pt-12 text-center flex flex-col">
            <h2 className="text-base font-semibold text-gray-900 mb-12 leading-tight">
              Are you sure you want to delete
              <br />
              the selected event{selectedEvents.length > 1 ? "s" : ""}?
            </h2>
            <div className="flex mt-auto">
              <button
