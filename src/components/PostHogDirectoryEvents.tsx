...
export const useDirectoryEvents = () => {
    ...
    const safeCapture = (eventName: string, properties: any = {}) => {
        ...
        // Data-quality guard
        if (properties.directory_name && properties.directory_name.match(/test|tesr|oiuytdftyj/i)) {
    ...
    const onDirectoryCreated = (properties: {
        directory_id: string;
        type?: string;
        status?: string;
        [key: string]: any;
    }) => {
        safeCapture('Directory Created', properties);
    };

    const onDirectoryUpdated = (properties: {
        directory_id: string;
        status?: string;
        [key: string]: any;
    }) => {
        safeCapture('Directory Updated', properties);
    };

    const onDirectoryDeleted = (properties: {
        directory_id: string;
        reason?: string;
        [key: string]: any;
    }) => {
        safeCapture('Directory Deleted', properties);
    };

    const onDirectoryViewed = (properties: {
        directory_id: string;
        [key: string]: any;
    }) => {
        safeCapture('Directory Viewed', properties);
    };
