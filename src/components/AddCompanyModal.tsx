...
interface CompanyFormData {
  ...
  finance_spoc: { name: string; designation: string; email: string; mobile: string };
  operation_spoc: { name: string; designation: string; email: string; mobile: string };
}
...
export const AddCompanyModal: React.FC<AddCompanyModalProps> = ({
  ...
  const validateForm = () => {
    ...
    if (formData.finance_spoc.email && !/\S+@\S+\.\S+/.test(formData.finance_spoc.email)) {
      newErrors.finance_email = 'Invalid email format';
    }

    if (formData.operation_spoc.email && !/\S+@\S+\.\S+/.test(formData.operation_spoc.email)) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formDataToSend.append('pms_company_setup[bill_to_address_attributes][address_type]', 'BillTo');
      ...
      formDataToSend.append('pms_company_setup[postal_address_attributes][address_type]', 'Postal');
      ...
      formDataToSend.append('pms_company_setup[finance_spoc_attributes][spoc_type]', 'Finance');
      ...
      formDataToSend.append('pms_company_setup[operation_spoc_attributes][spoc_type]', 'Operation');
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose} modal={false}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto bg-white z-50" aria-describedby="add-company-dialog-description">
        <DialogHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
          <DialogTitle className="text-lg font-semibold text-gray-900">ADD NEW COMPANY</DialogTitle>
          <Button
            variant="ghost"
            size="sm"
            onClick={handleClose}
