...
interface AddPlantDetailDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSubmit: (data: PlantDetailForm) => void;
}

export const AddPlantDetailDialog: React.FC<AddPlantDetailDialogProps> = ({
  open,
  onOpenChange,
  onSubmit,
}) => {
  ...
  const [formData, setFormData] = useState<PlantDetailForm>({
    plant_name: '',
    site_name: '',
    valuation_area: '',
    plant_category: '',
  ...
  const handleInputChange = (field: keyof PlantDetailForm, value: string) => {
    ...
    <Dialog open={open} onClose={handleClose}>
      <DialogContent className="sm:max-w-2xl">
        <DialogHeader className="flex flex-row items-center justify-between space-y-0 mb-4">
          <h1 className="text-lg font-semibold">Add Plant Detail</h1>
          <Button
            variant="ghost"
            size="sm"
            onClick={handleClose}
