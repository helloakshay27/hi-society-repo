...
export const useVisitorEvents = () => {
    ...
    const safeCapture = (eventName: string, properties: any = {}) => {
        ...
        // Data-quality guard
        if (properties.visitor_name && properties.visitor_name.match(/test|tesr|oiuytdftyj/i)) {
    ...
    const onVisitorCreated = (properties: {
        visitor_id: string;
        type?: string;
        status?: string;
        [key: string]: any;
    }) => {
        safeCapture('Visitor Created', properties);
    };

    const onVisitorUpdated = (properties: {
        visitor_id: string;
        status?: string;
        [key: string]: any;
    }) => {
        safeCapture('Visitor Updated', properties);
    };

    const onVisitorDeleted = (properties: {
        visitor_id: string;
        reason?: string;
        [key: string]: any;
    }) => {
        safeCapture('Visitor Deleted', properties);
    };

    const onVisitorViewed = (properties: {
        visitor_id: string;
        [key: string]: any;
    }) => {
        safeCapture('Visitor Viewed', properties);
    };
