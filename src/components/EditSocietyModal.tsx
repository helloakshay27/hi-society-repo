...
export const EditSocietyModal: React.FC<EditSocietyModalProps> = ({
  isOpen,
  onClose,
  onSuccess,
  societyId,
  ...
  // Debug
  useEffect(() => {
    console.log("🟡 EditSocietyModal state:", { isOpen, societyId, canEdit });
  ...
  // Fetch society details when modal opens
  useEffect(() => {
    if (isOpen && societyId) {
      fetchSocietyDetails();
      fetchCompanies();
  ...
  const fetchSocietyDetails = async () => {
    try {
      ...
      if (!response.ok) throw new Error("Failed to fetch society details");
      ...
      setFormData({
        ...
        app_id: data.app_id || "",
        url: data.url || "",
        no_of_devices: data.no_of_devices,
        device_rental_type: data.device_rental_type || "",
        device_rental_rate: data.device_rental_rate,
        ...
        billing_term: data.billing_term || "",
        billing_rate: data.billing_rate,
        billing_cycle: data.billing_cycle || "",
        start_date: data.start_date || "",
        end_date: data.end_date || "",
        registration: data.registration || "",
        approve: data.approve === 1 ? "Yes" : "",
        comment: data.comment || "",
        project_type: data.project_type || "",
        allow_view_toggle: data.allow_view_toggle || false,
        active: data.active === 1,
        description: data.description || "",
        IsDelete: data.IsDelete || 0,
        company_id: data.company_id,
        super_society_id: data.super_society_id,
        ivr_enabled: data.ivr_enabled ?? false,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log("🔄 Updating society:", payload);
