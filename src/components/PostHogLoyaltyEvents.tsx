...
export const useLoyaltyEvents = () => {
    ...
    const safeCapture = (eventName: string, properties: any = {}) => {
        ...
        // Data-quality guard
        if (properties.loyalty_name && properties.loyalty_name.match(/test|tesr|oiuytdftyj/i)) {
    ...
    const onLoyaltyCreated = (properties: {
        loyalty_id: string;
        type?: string;
        status?: string;
        [key: string]: any;
    }) => {
        safeCapture('Loyalty Created', properties);
    };

    const onLoyaltyUpdated = (properties: {
        loyalty_id: string;
        status?: string;
        [key: string]: any;
    }) => {
        safeCapture('Loyalty Updated', properties);
    };

    const onLoyaltyDeleted = (properties: {
        loyalty_id: string;
        reason?: string;
        [key: string]: any;
    }) => {
        safeCapture('Loyalty Deleted', properties);
    };

    const onLoyaltyViewed = (properties: {
        loyalty_id: string;
        [key: string]: any;
    }) => {
        safeCapture('Loyalty Viewed', properties);
    };
