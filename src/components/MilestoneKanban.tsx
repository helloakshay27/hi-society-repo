...
const MilestoneKanban = () => {
    ...
    const handleDragEnd = useCallback((event: DragEndEvent) => {
        ...
        // Check if we're dragging a milestone over a kanban column
        if (activeData?.type === "MILESTONE" && overData?.type === "KANBAN_COLUMN") {
            const milestoneId = activeData.projectId;
            const milestone = activeData.project;
            ...
            if (milestone.status === apiStatus || (newStatus === "open" && milestone.status === "open")) {
                return;
            }

            // Store the dropped milestone with its new status for optimistic update
            setDroppedProjects(prev => ({
                ...prev,
                [milestoneId]: apiStatus
            }));

            // Call API to update milestone status
            if (token && baseUrl && milestoneId) {
                dispatch(updateMilestoneStatus({
                    ...
                    // Refresh milestones after status update
                    dispatch(fetchMilestones({ token, baseUrl, id })).unwrap().then((response) => {
                        setData(response);
                        // Clear droppedProjects after successful refresh since data is now updated
                        setDroppedProjects({});
                    }).catch((error) => {
                        console.log(error);
        ...
        // Check if we're dragging a task over a kanban column
        if (activeData?.type === "TASK" && overData?.type === "KANBAN_COLUMN") {
            ...
            // Don't update if dropping in the same column
            if (task.status === apiStatus || (newStatus === "open" && task.status === "open")) {
                return;
            }

            // Store the dropped task with its new status for optimistic update
            ...
            if (token && baseUrl && taskId) {
                dispatch(editProjectTask({
                    ...
                    // Refresh milestones after task status update
                    dispatch(fetchMilestones({ token, baseUrl, id })).unwrap().then((response) => {
                        setData(response);
                        // Clear droppedTasks after successful refresh since data is now updated
                        setDroppedTasks({});
                    }).catch((error) => {
                        console.log(error);
                        // On error, remove the optimistic update
    ...
    return (
        <div className="relative mx-4">
            ...
            >
                <div
                    className="h-[80%] flex items-start gap-1 max-w-full overflow-x-auto overflow-y-auto flex-nowrap"
                    style={{ height: "75vh" }}
                >
                    {cardsTitle.map((card) => {
                        const cardStatus = card.title.toLowerCase().replace(/\s+/g, "_");

                        const filteredMilestone =
                            Array.isArray(data) &&
                            data.filter((milestone) => {
                                // Check if this milestone was dropped with a new status
                                const droppedStatus = droppedProjects[milestone.id];
                                const milestoneStatus = droppedStatus || milestone.status;

                                // Map "open" to "open" for comparison (no normalization needed)
                                const normalizedCardStatus = cardStatus;

                                return milestoneStatus === normalizedCardStatus;
                            });

                        // Collect all tasks that should appear in this column
                        const allTasksInColumn: Array<{ task: any; milestoneId: number }> = [];
                        if (Array.isArray(data)) {
                            data.forEach((milestone) => {
                                const milestoneTasks = milestone.task_managements || [];
                                milestoneTasks.forEach((task: any) => {
                                    // Check if this task was dropped with a new status
                                    const droppedTaskStatus = droppedTasks[task.id];
                                    const taskStatus = droppedTaskStatus || task.status;

                                    // Check if task belongs to this column
                                    const belongsToColumn = cardStatus === "open"
                                        ? taskStatus === "open"
                                        : taskStatus === cardStatus;

                                    if (belongsToColumn) {
                                        allTasksInColumn.push({ task, milestoneId: milestone.id });
                                    }
                                });
                            });
                        }
                                ...
                                {filteredMilestone.length > 0 || allTasksInColumn.length > 0 ? (
                                    <>
                                        ...
                                        {(filteredMilestone || []).map((milestone) => {
                                            const milestoneTasks = milestone.task_managements || [];
                                            const visibleTasks = milestoneTasks.filter((task: any) => {
                                                // Check if this task was dropped with a new status
                                                const droppedTaskStatus = droppedTasks[task.id];
                                                const taskStatus = droppedTaskStatus || task.status;

                                                // Only show tasks in the milestone if milestone is in this column
                                                // Otherwise tasks will be shown separately below
                                                const milestoneInColumn = cardStatus === "open"
                                                    ? (droppedProjects[milestone.id] || milestone.status) === "open"
                                                    : (droppedProjects[milestone.id] || milestone.status) === cardStatus;

                                                if (!milestoneInColumn) return false;

                                                return cardStatus === "open" ? taskStatus === "open" : taskStatus === cardStatus;
                                            });
                                        ...
                                        {/* Render standalone tasks that are in this column but their milestone is in another column */}
                                        {allTasksInColumn
                                            .filter(({ task, milestoneId }) => {
                                                const milestone = data.find((m: any) => m.id === milestoneId);
                                                if (!milestone) return false;

                                                const milestoneStatus = droppedProjects[milestone.id] || milestone.status;
                                                const milestoneInColumn = cardStatus === "open"
                                                    ? milestoneStatus === "open"
                                                    : milestoneStatus === cardStatus;

                                                // Only show task standalone if milestone is NOT in this column
                                                return !milestoneInColumn;
                                            })
                                            .map(({ task, milestoneId }) => (
                                                <div
                                                    key={`standalone-task-${task.id}`}
                                                    id={`task-${task.id}`}
                                                    className="relative"
                                                >
                                                    <MilestoneTaskCard
                ...
                <DragOverlay>
                    {draggedProjectId ? (
                        Array.isArray(data) &&
                            data.find(milestone => milestone.id.toString() === draggedProjectId) ? (
                            <div className="w-60 opacity-100">
                                <MilestoneCard
                                    milestone={data.find(milestone => milestone.id.toString() === draggedProjectId)}
                                    toggleTaskCard={() => { }}
                        ...
                        (() => {
                            ...
                            for (const milestone of data) {
                                const task = milestone.task_managements?.find((t: any) => t.id.toString() === draggedTaskId);
                                if (task) {
                                    return (
                                        <div className="w-60 opacity-100" key={`overlay-task-${task.id}`}>
                                            <MilestoneTaskCard
                                                task={task}
                                                parentMilestoneId={milestone.id}
                                            />
