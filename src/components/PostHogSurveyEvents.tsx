...
export const useSurveyEvents = () => {
    ...
    const safeCapture = (eventName: string, properties: any = {}) => {
        ...
        // Data-quality guard
        if (properties.survey_name && properties.survey_name.match(/test|tesr|oiuytdftyj/i)) {
    ...
    const onSurveyCreated = (properties: {
        survey_id: string;
        type?: string;
        status?: string;
        [key: string]: any;
    }) => {
        safeCapture('Survey Created', properties);
    };

    const onSurveyUpdated = (properties: {
        survey_id: string;
        status?: string;
        [key: string]: any;
    }) => {
        safeCapture('Survey Updated', properties);
    };

    const onSurveyDeleted = (properties: {
        survey_id: string;
        reason?: string;
        [key: string]: any;
    }) => {
        safeCapture('Survey Deleted', properties);
    };

    const onSurveyViewed = (properties: {
        survey_id: string;
        [key: string]: any;
    }) => {
        safeCapture('Survey Viewed', properties);
    };
