...
export const useFitoutEvents = () => {
    ...
    const safeCapture = (eventName: string, properties: any = {}) => {
        ...
        // Data-quality guard
        if (properties.fitout_name && properties.fitout_name.match(/test|tesr|oiuytdftyj/i)) {
    ...
    const onFitoutCreated = (properties: {
        fitout_id: string;
        type?: string;
        status?: string;
        [key: string]: any;
    }) => {
        safeCapture('Fitout Created', properties);
    };

    const onFitoutUpdated = (properties: {
        fitout_id: string;
        status?: string;
        [key: string]: any;
    }) => {
        safeCapture('Fitout Updated', properties);
    };

    const onFitoutDeleted = (properties: {
        fitout_id: string;
        reason?: string;
        [key: string]: any;
    }) => {
        safeCapture('Fitout Deleted', properties);
    };

    const onFitoutViewed = (properties: {
        fitout_id: string;
        [key: string]: any;
    }) => {
        safeCapture('Fitout Viewed', properties);
    };
