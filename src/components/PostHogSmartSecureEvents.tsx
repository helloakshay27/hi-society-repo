...
export const useSmartSecureEvents = () => {
    ...
    const safeCapture = (eventName: string, properties: any = {}) => {
        ...
        // Data-quality guard
        if (properties.smartsecure_name && properties.smartsecure_name.match(/test|tesr|oiuytdftyj/i)) {
    ...
    const onSmartSecureCreated = (properties: {
        smartsecure_id: string;
        type?: string;
        status?: string;
        [key: string]: any;
    }) => {
        safeCapture('SmartSecure Created', properties);
    };

    const onSmartSecureUpdated = (properties: {
        smartsecure_id: string;
        status?: string;
        [key: string]: any;
    }) => {
        safeCapture('SmartSecure Updated', properties);
    };

    const onSmartSecureDeleted = (properties: {
        smartsecure_id: string;
        reason?: string;
        [key: string]: any;
    }) => {
        safeCapture('SmartSecure Deleted', properties);
    };

    const onSmartSecureViewed = (properties: {
        smartsecure_id: string;
        [key: string]: any;
    }) => {
        safeCapture('SmartSecure Viewed', properties);
    };

    return {
        onSmartSecureCreated,
        onSmartSecureUpdated,
        onSmartSecureDeleted,
        onSmartSecureViewed
    };
};
