...
export const useAppointmentzEvents = () => {
    ...
    const safeCapture = (eventName: string, properties: any = {}) => {
        ...
        // Data-quality guard
        if (properties.appointmentz_name && properties.appointmentz_name.match(/test|tesr|oiuytdftyj/i)) {
    ...
    const onAppointmentzCreated = (properties: {
        appointmentz_id: string;
        type?: string;
        status?: string;
        [key: string]: any;
    }) => {
        safeCapture('Appointmentz Created', properties);
    };

    const onAppointmentzUpdated = (properties: {
        appointmentz_id: string;
        status?: string;
        [key: string]: any;
    }) => {
        safeCapture('Appointmentz Updated', properties);
    };

    const onAppointmentzDeleted = (properties: {
        appointmentz_id: string;
        reason?: string;
        [key: string]: any;
    }) => {
        safeCapture('Appointmentz Deleted', properties);
    };

    const onAppointmentzViewed = (properties: {
        appointmentz_id: string;
        [key: string]: any;
    }) => {
        safeCapture('Appointmentz Viewed', properties);
    };

    return {
        onAppointmentzCreated,
        onAppointmentzUpdated,
        onAppointmentzDeleted,
        onAppointmentzViewed
    };
};
