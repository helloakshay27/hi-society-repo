...
export const useProjectEvents = () => {
    ...
    const safeCapture = (eventName: string, properties: any = {}) => {
        ...
        // Data-quality guard
        if (properties.project_name && properties.project_name.match(/test|tesr|oiuytdftyj/i)) {
    ...
    const onProjectCreated = (properties: {
        project_id: string;
        type?: string;
        status?: string;
        [key: string]: any;
    }) => {
        safeCapture('Project Created', properties);
    };

    const onProjectUpdated = (properties: {
        project_id: string;
        status?: string;
        [key: string]: any;
    }) => {
        safeCapture('Project Updated', properties);
    };

    const onProjectDeleted = (properties: {
        project_id: string;
        reason?: string;
        [key: string]: any;
    }) => {
        safeCapture('Project Deleted', properties);
    };

    const onProjectViewed = (properties: {
        project_id: string;
        [key: string]: any;
    }) => {
        safeCapture('Project Viewed', properties);
    };
