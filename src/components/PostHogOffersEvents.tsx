...
export const useOffersEvents = () => {
    ...
    const safeCapture = (eventName: string, properties: any = {}) => {
        ...
        // Data-quality guard
        if (properties.offers_name && properties.offers_name.match(/test|tesr|oiuytdftyj/i)) {
    ...
    const onOffersCreated = (properties: {
        offers_id: string;
        type?: string;
        status?: string;
        [key: string]: any;
    }) => {
        safeCapture('Offers Created', properties);
    };

    const onOffersUpdated = (properties: {
        offers_id: string;
        status?: string;
        [key: string]: any;
    }) => {
        safeCapture('Offers Updated', properties);
    };

    const onOffersDeleted = (properties: {
        offers_id: string;
        reason?: string;
        [key: string]: any;
    }) => {
        safeCapture('Offers Deleted', properties);
    };

    const onOffersViewed = (properties: {
        offers_id: string;
        [key: string]: any;
    }) => {
        safeCapture('Offers Viewed', properties);
    };
