...
const ProjectConfigurationList = () => {
  ...
  const totalFiltered = filteredConfigurations.length;
      ...
      <div className="p-6 max-w-full h-[calc(100vh-50px)] overflow-y-auto">
        ...
        <div className="mb-6">
          <div className="flex items-center space-x-2 text-sm text-gray-600 mb-2">
            ...
            <ChevronRight className="w-4 h-4" />
            <span className="text-[#C72030] font-medium">Project Configuration</span>
          </div>
...
<div className="flex items-center justify-between mb-4">
   {(projectConfigurationPermission.create === "true" || true) && (
    <button
      ...
      <svg width={16} height={16} viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
          <div className="bg-[#F6F4EE] px-6 py-4 border-b border-gray-200">
            <h3 className="text-lg font-bold text-gray-900">Project Configuration List</h3>
          </div>
          <div className="p-6">
            {loading ? (
                  ...
                  <Table className="border-separate">
                    <TableHeader>
                      <TableRow className="hover:bg-gray-50" style={{ backgroundColor: "#e6e2d8" }}>
                        ...
                        <TableHead className="font-semibold text-gray-900 py-3 px-4 border-r text-center" style={{ borderColor: "#fff", width: "150px" }}>Attachment</TableHead>
                    ...
                    <TableBody>
                      {paginatedData.length > 0 ? (
                        paginatedData.map((config, index) => (
                          <TableRow key={config.id} className="hover:bg-gray-50 transition-colors">
                            <TableCell className="py-3 px-4">
                              {projectConfigurationPermission.update === "true" && (
                                <button
                                  onClick={() => navigate(`/setup-member/project-config-edit/${config.id}`)}
                            ...
                            <TableCell className="py-3 px-4">{config.name || "-"}</TableCell>
                            <TableCell className="py-3 px-4">
                              <div className="flex justify-center">
                                {config.attachfile?.document_file_name ? (
                                  config.attachfile.document_content_type?.startsWith("image/") ? (
                                    <img
                  ...
                  <div className="flex items-center justify-between mt-6 px-3">
                    ...
                    <div>
                      <p className="text-sm text-gray-600">
                        Showing <span className="font-medium">{totalFiltered === 0 ? 0 : startIndex + 1}</span> to <span className="font-medium">{Math.min(startIndex + paginatedData.length, totalFiltered)}</span> of <span className="font-medium">{totalFiltered}</span> entries
                      </p>
                    </div>
                  </div>
                )}
