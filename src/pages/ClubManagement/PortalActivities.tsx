...
const MOCK_ROWS: PortalActivityRow[] = [];

const PortalActivities: React.FC = () => {
  ...
  const renderRow = (row: PortalActivityRow) => ({
    date: (
    ...
    activityDetails: (
      <span className="text-[13px] text-[#111827]">{row.activityDetails}</span>
    ),
    description: (
      ...
      <div className="overflow-hidden rounded-lg border border-[#EAECF0] bg-white">
        ...
        <div className="p-4">
          <EnhancedTaskTable
            ...
            columns={columns}
            renderRow={renderRow}
            storageKey="portal-activities-v1"
            hideTableExport
            hideTableSearch
            hideColumnsButton
