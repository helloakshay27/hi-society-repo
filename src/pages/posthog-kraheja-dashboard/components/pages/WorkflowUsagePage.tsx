...
import { WorkflowFunnel } from '../../../posthog-runwal-dashboard/components/workflow/WorkflowFunnel';
...
/** K Raheja Corp Homes — Workflow Usage tab, matching the wireframe's pgFlows section, wired to live Runwal-family PostHog APIs. */
export const WorkflowUsagePage: React.FC<WorkflowUsagePageProps> = ({ filters, benchmarks, onBenchmarkChange, sitesSettled = true }) => {
  const [selectedModule, setSelectedModule] = useState<string | null>(null);

  const { data: moduleTreeData, isLoading: isModulesLoading } = useModuleTree(filters, sitesSettled);
  const { data: wfData, isLoading: isWfLoading, isError: isWfError, error: wfError, refetch: refetchWf } = useWorkflowUsage(
    filters,
    selectedModule,
    null,
    sitesSettled
  ...
  const fAdoptDisplay = fAdopt != null ? `${Math.round(fAdopt <= 1 ? fAdopt * 100 : fAdopt)}%` : isWfLoading ? '...' : '—';
  const fAdoptDelta = kpis?.f_adopt?.delta_pct;

  const fComp = kpis?.f_comp?.value;
  const fCompDisplay = fComp != null ? `${Math.round(fComp <= 1 ? fComp * 100 : fComp)}%` : isWfLoading ? '...' : '—';
  const fCompDelta = kpis?.f_comp?.delta_pct;

  const fStep = kpis?.f_step?.value;
  const fStepDisplay = fStep != null ? `${Math.round(fStep <= 1 ? fStep * 100 : fStep)}%` : isWfLoading ? '...' : '—';

  const fVol = kpis?.f_vol?.value;
  const fVolDisplay = fVol != null ? fVol.toLocaleString() : isWfLoading ? '...' : '—';
  ...
  return (
    <section className="page on" id="pgFlows">
      <div className="section-head">
        ...
        <span className="sd">Resident/prospect completion of key journeys per module, all-modules comparison, and where users navigate &amp; exit.</span>
      </div>

      <ModuleNav modules={moduleList} selectedModule={selectedModule} onSelectModule={setSelectedModule} isLoading={isModulesLoading} />

      {isWfError && (
        <div style={{ marginBottom: '16px' }}>
          <ErrorState title="Failed to load Workflow Usage metrics" error={wfError} onRetry={() => refetchWf()} />
        </div>
      )}
      ...
      <div className="tiles" style={{ gridTemplateColumns: 'repeat(4, 1fr)' }}>
        ...
        <KpiTile infoKey="F4" label="Usage Volume" val={fVolDisplay} dir="up" delta="this period" sub="workflow completions" noTarget={true} isLoading={isWfLoading} />
      </div>
