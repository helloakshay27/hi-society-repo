...
const LoanManagerList = () => {
  ...
  const totalFiltered = filteredLoanManagers.length;
      ...
      <div className="p-6 max-w-full h-[calc(100vh-50px)] overflow-y-auto">
        ...
        <div className="mb-6">
          <div className="flex items-center space-x-2 text-sm text-gray-600 mb-2">
            ...
            <ChevronRight className="w-4 h-4" />
            <span className="text-[#C72030] font-medium">Loan Managers</span>
          </div>
          <h1 className="text-2xl font-bold text-gray-900">LOAN MANAGERS</h1>
        </div>
        ...
        <div className="flex justify-end items-center gap-3 mb-4">
          ...
          {loanManagerPermissions.create === "true" && (
            <button
              ...
              onClick={() => navigate("/setup-member/loan-manager-add")}
            >
              <svg xmlns="http://www.w3.org/2000/svg" width={20} height={20} fill="currentColor" viewBox="0 0 16 16">
                <path d="M8 4a.5.5 0 0 1 .5.5v3h3a.5.5 0 0 1 0 1h-3v3a.5.5 0 0 1-1 0v-3h-3a.5.5 0 0 1 0-1h3v-3A.5.5 0 0 1 8 4"></path>
              </svg>
              <span>Add Loan Manager</span>
            </button>
          )}
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
          <div className="bg-[#F6F4EE] px-6 py-4 border-b border-gray-200">
            <h3 className="text-lg font-bold text-gray-900">Loan Manager List</h3>
          </div>
          <div className="p-6">
            {loading ? (
              ...
              <>
                  ...
                  <Table className="border-separate">
                    <TableHeader>
                      <TableRow className="hover:bg-gray-50" style={{ backgroundColor: "#e6e2d8" }}>
                        ...
                        <TableHead className="font-semibold text-gray-900 py-3 px-4 border-r" style={{ borderColor: "#fff", minWidth: "180px" }}>Name</TableHead>
                        <TableHead className="font-semibold text-gray-900 py-3 px-4 border-r" style={{ borderColor: "#fff", minWidth: "200px" }}>Email</TableHead>
                        <TableHead className="font-semibold text-gray-900 py-3 px-4 border-r" style={{ borderColor: "#fff", width: "140px" }}>Mobile</TableHead>
                        <TableHead className="font-semibold text-gray-900 py-3 px-4 border-r" style={{ borderColor: "#fff", width: "120px" }}>Project ID</TableHead>
                    ...
                    <TableBody>
                      {displayedLoanManagers.length > 0 ? (
                        displayedLoanManagers.map((loanManager, index) => (
                          <TableRow key={loanManager.id} className="hover:bg-gray-50 transition-colors">
                            <TableCell className="py-3 px-4">
                              {loanManagerPermissions.update === "true" && (
                                <button
                                  onClick={() => navigate(`/setup-member/loan-manager-edit/${loanManager.id}`)}
                            ...
                            <TableCell className="py-3 px-4">{loanManager.name || "-"}</TableCell>
                            <TableCell className="py-3 px-4">{loanManager.email || "-"}</TableCell>
                            <TableCell className="py-3 px-4">{loanManager.mobile || "-"}</TableCell>
                            <TableCell className="py-3 px-4">{loanManager.project_id || "-"}</TableCell>
                            <TableCell className="py-3 px-4">
                        ...
                        <TableRow>
                          <TableCell colSpan={7} className="text-center py-12 text-gray-500">
                            {searchQuery ? (
                              <div>
                                <p className="text-lg mb-2">No loan managers found</p>
                                <p className="text-sm text-gray-400">Try adjusting your search criteria</p>
                              </div>
                            ) : (
                              "No loan managers found"
                            )}
                          </TableCell>
                        </TableRow>
                      )}
                ...
                {/* Pagination */}
                {displayedLoanManagers.length > 0 && totalPages > 1 && (
                  <div className="flex items-center justify-between mt-6 px-3">
                    ...
                    <div>
                      <p className="text-sm text-gray-600">
                        Showing <span className="font-medium">{totalFiltered === 0 ? 0 : startIndex + 1}</span> to <span className="font-medium">{Math.min(startIndex + displayedLoanManagers.length, totalFiltered)}</span> of <span className="font-medium">{totalFiltered}</span> entries
                      </p>
                    </div>
                  </div>
                )}
