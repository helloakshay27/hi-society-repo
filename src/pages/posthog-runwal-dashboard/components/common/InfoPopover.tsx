 ...
 *  Portaled to document.body; `.info-pop` is a global rule in dashboard.css for that reason. */
export function InfoPopover() {
  const { infoPopover, closeInfoPopover } = useDashboard();
  const popRef = useRef<HTMLDivElement>(null);
  const [pos, setPos] = useState<{ left: number; top: number } | null>(null);

  // Position popover near the button, ensuring it stays in viewport
  useLayoutEffect(() => {
    ...
    // Position below button, fallback to above if not enough space
  ...
  useEffect(() => {
    ...
    const onDocClick = (e: MouseEvent) => {
      ...
      if (!target.closest('.info-pop') && !target.closest('.info-btn')) {
        closeInfoPopover();
      }
    };
  ...
  return createPortal(
    ...
    >
      ...
      <div className="ipf">
        <span className="ipfk">How it's calculated</span>
        {info.f}
      </div>
