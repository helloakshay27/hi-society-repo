 ...
 * hooks, verbatim. The only real difference: Runwal CP has no app_id at all,
 * so every request identifies itself with a fixed `project_code=RE-CP01`
...
function PosthogRunwalCpDashboardContent() {
  ...
  // Keep the URL carrying ?project_code=RE-CP01 so the handful of API helpers
  // that read it directly off window.location (getFm, fetchAllowedSites) pick
  // it up the same way the app_id-based dashboard relies on ?app_id=... being
  // present — this is a non-navigating URL-bar update, not a redirect.
  useEffect(() => {
    try {
  ...
  const filters: DashboardFilters = useMemo(() => {
    ...
    return {
      ...
      appId: undefined,
      projectCode: RUNWAL_CP_PROJECT_CODE,
    };
  }, [accountSiteId, devPlatform, rangeFrom, rangeTo, dynamicTenantUrl]);
...
export const PosthogRunwalCpDashboard: React.FC = () => {
  return (
    <DashboardProvider>
      <PosthogRunwalCpDashboardContent />
    </DashboardProvider>
  );
};

export default PosthogRunwalCpDashboard;
