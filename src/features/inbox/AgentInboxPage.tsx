...
const AgentInboxPage = () => {
  ...
  // Agent name passed via navigation state from dashboard
  const agentName: string =
    (location.state as { agentName?: string })?.agentName ?? "Agent";

  const [activeStatus, setActiveStatus] = useState<string>("pending");
  const [preview, setPreview] = useState<InboxMessage | null>(null);

  const queryKey = ["inboxMessages", userId, activeStatus];

  const { data, isLoading, isError, refetch } = useQuery({
    queryKey,
    queryFn: () =>
      inboxMessageApi.list({ assigned_user_id: userId, status: activeStatus }),
    enabled: !!userId,
  });
      ...
      >
        {STATUS_TABS.map(({ key, label }) => (
          <button
            ...
            style={{
              color:
                activeStatus === key ? "var(--coral)" : "var(--text-muted)",
              borderBottom:
                activeStatus === key
                  ? "2px solid var(--coral)"
              ...
              {messages.length === 0 && (
                <tr>
                  <td
                    ...
                    style={{ color: "var(--text-muted)" }}
                  >
                    No messages with status "{activeStatus.replace("_", " ")}".
                  </td>
                </tr>
              )}
              {messages.map((msg) => (
                ...
                >
                  ...
                  >
                    ...
                    {msg.status === "ticket_created" && msg.ticket && (
                      <Link
