...
class NewScratchCardApi {
    ...
    initialize(orgId?: string, token?: string) {
        ...
        if (orgId && token) {
            this.orgId = orgId;
            this.token = token;
            console.warn("🔧 ScratchCard API Initialize - org_id:", orgId, "token:", token ? "✓" : "✗");
            return;
        }
        ...
        console.warn("🔧 ScratchCard API Initialize from URL - org_id:", urlOrgId, "token:", urlToken ? "✓" : "✗");
    ...
    async getContests(): Promise<ScratchContest[]> {
        try {
            ...
            const message = apiError.response?.data?.message || apiError.message || "Failed to fetch contests";
    ...
    async getContestById(contestId: string | number): Promise<ScratchContest> {
        try {
            ...
            if (this.useLocalData) {
                ...
                const contest = dummyScratchContests.find(c => c.id === Number(contestId));
                if (!contest) {
            ...
            const apiError = error as ApiError;
            const message = apiError.response?.data?.message || apiError.message || "Failed to fetch contest";
            throw new Error(message);
        }
    }
    ...
    async playContest(contestId: string | number): Promise<ScratchPlayResult> {
        try {
            ...
            if (this.useLocalData) {
                ...
                const totalProbability = contest.prizes.reduce((sum, p) => sum + p.probability_value, 0);
            ...
            const response = await baseClient.post<ScratchPlayResult>(url, {}, { params });
            ...
            const apiError = error as ApiError;
            const message = apiError.response?.data?.message || apiError.message || "Failed to play contest";
            throw new Error(message);
        }
    }
}
